 ...
 */
object UpdateFunctionVerson_One {
  ...
  val num = new java.util.Random()
   ...
   // println("count:#########:"+byCountyBargains.count())
    ...
    val lgfinal = byCountyBargains.flatMap{line=>
      ...
      JedisUtils.initPool
      val j: Jedis = JedisUtils.getJedis
      val collectMapinitializesSC= j.hgetAll(cityName+"AVMINIT")
    ...
    //  println("collectMapCommunity!!!!!!!!!!!!:"+collectMapCommunity.size())
     ...
     // println("okokokokokoooooooooooooooooooooooooooooooo")
      line._2.map{line=>
        ...
        val bargainForImpal = bargainDetialArrays(0)+"\t"+
          bargainDetialArrays(1)+"\t"+
          bargainDetialArrays(2)+"\t"+
          bargainDetialArrays(4)+"\t"+
          bargainDetialArrays(7)+"\t"+
          bargainDetialArrays(8)+"\t"+
          bargainDetialArrays(5)+"\t"+
          bargainDetialArrays(6)+"\t"+
          bargainDetialArrays(11)+"\t"+
          bargainDetialArrays(10)+"\t"+
          bargainDetialArrays(9)



        val bargainID = bargainDetialArrays(0)
        val pcommunityID = bargainDetialArrays(3)

        val pfloor = bargainDetialArrays(7).toInt
        val pTotalFloor= bargainDetialArrays(8).toInt
        val pLocation_Longitude:Double=bargainDetialArrays(9).toDouble
        val pLocation_Latitude:Double=bargainDetialArrays(10).toDouble
        val psquare =  bargainDetialArrays(5).toDouble
        val pfaceTo =bargainDetialArrays(6)
        val pbuildYear = bargainDetialArrays(11)


        val cityName =  bargainDetialArrays(1)
    ...
    val lg3 = lgfinal.filter(x => x._6 != "").flatMap{x =>
