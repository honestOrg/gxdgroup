...
*/
object Initial22CommunityDriver {
  ...
  def main(args: Array[String]){
    ...
    GetAllCommunity16.cartesian(GetAllCommunity16).filter{line =>
     line._1.split("\t")(0)!=line._2.split("\t")(0)
      ...
      line =>
        ...
        val distance = AVMUtils.GetDistance(target1(5).toDouble,
          ...
          target2(5).toDouble,
          target2(4).toDouble)
     // println("target1(4):"+target1(5)+","+target1(4)+","+target2(5)+","+target2(4))
   ...
   //   println("测试系数:"+m_Setting("测试系数").maxDistance)
      line._1<m_Setting("测试系数").maxDistance}.map(line =>
      line._2).saveAsTextFile("lg/gxd/cartesianCommunity.txt")
