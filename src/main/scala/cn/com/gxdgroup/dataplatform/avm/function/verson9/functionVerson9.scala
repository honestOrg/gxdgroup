package cn.com.gxdgroup.dataplatform.avm.function.verson9
...
import cn.com.gxdgroup.dataplatform.avm.model.Index2
import org.apache.commons.lang.StringUtils
import scala.collection.JavaConversions._
/**
 * Created by ThinkPad on 14-7-5.
 */
object functionVerson9 {



  def main(args: Array[String]) {
println("ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
    val sc = new SparkContext("spark://cloud40:7077", "gxd",
      System.getenv("SPARK_HOME"), SparkContext.jarOfClass(this.getClass))


    val GetAllBargainTextFile = sc.textFile(args(0),args(1).toInt)
    val cityArrays = args(2)
   // JedisUtils.initPool
    val j1 = new Jedis("localhost",6379)
    val threadholdDetal = j1.hget("THREADHOLD", "测试阈值")
println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa:"+threadholdDetal)
    ...
    //批量计算开始
  val   lgfinal = GetAllBargainTextFile.mapPartitions{iter=>{
println("jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj")


 //     JedisUtils.initPool
      val j2 = new Jedis("localhost",6379)
      iter.map{line=>
      ...
      val targetCommunityList = j2.hget(cityArrays+"小区表",pcommunityID)
      val bargainVal = j2.hget(cityArrays+"案例表_TEST",pcommunityID)
      var indexListByRedis= j2.lrange(cityArrays.replace("市",""),0,-1)
      ...
      if (firstResultList.size < 5) {
         ...
         */
          case true => List()
          case false =>
            val array42Fields = targetCommunityList.split("\t")
            //*******************************

            //***********************************
            var psimilarVal =  j2.hget(cityArrays+"AVMINIT",pcommunityID)
            if (psimilarVal != null) {
              psimilarVal.split(",").filter {
                x =>
                  ...
                  val communityDetail = j2.hget(cityArrays+"小区表",distanceAndCommID(1)).split("\t")
                  (
        ...
        similarCommunityListReal.isEmpty match {
          ...
          case false =>
            ...
            similarCommunityListReal.map(scl => {
              ...
              val bargainVal2 = j2.hget(cityArrays+"案例表_TEST",scl._1)
              if (bargainVal2 != null) {
      ...
      val beijingIndexList = indexListByRedis.map {line =>
